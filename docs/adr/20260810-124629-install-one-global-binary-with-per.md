...
the binary at `.claude/bin/dlog`, hooks in that repo's `.claude/settings.json`.
That is one wiring step per repository, and the binary duplicated into every one
of them.

The requirement that broke it was wanting dlog available in any repository
without repeating the setup.

## Decision

One binary at `~/.local/bin/dlog`, hooks declared once in
`~/.claude/settings.json`, and a repository opts in by having a `.dlog/`
...
to act as a switch. `dlog new` creates `.dlog/`, so writing a first record turns a
...
`$CLAUDE_PROJECT_DIR/.dlog` and warns about a missing binary only in a repository
that did opt in.
